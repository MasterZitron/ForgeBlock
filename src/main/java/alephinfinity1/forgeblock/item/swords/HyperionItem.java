...
public class HyperionItem extends FBSwordItem implements IDamageAbilityItem, IRequirementItem {

	public HyperionItem(Properties props, FBTier tier, double attackDamageIn, double strengthIn, double critChanceIn, double critDamageIn) {
		super(props, tier, attackDamageIn, strengthIn, critChanceIn, critDamageIn);
	}

	public HyperionItem(Properties props, FBTier tier, Multimap<String, AttributeModifier> modifiers) {
	...
	public AbilityResultType activateAbility(World world, PlayerEntity player, ItemStack stack) {
		...
		player.playSound(SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.PLAYERS, 0.5f, 1.4f);
		// damage all entities via magic in a 12 blocks radius
		AxisAlignedBB bound = new AxisAlignedBB(player.getPositionVec().add(-6.0d, -3.0d, -6.0d), player.getPositionVec().add(6.0d, 3.0d, 6.0d));
	...
	public double getAbilityCost(ItemStack stack, PlayerEntity player) {
		...
		else return player.isCreative() ? 0 : (300.0D / (100.0D + player.getAttribute(FBAttributes.MANA_EFFICIENCY).getValue()) * 100.0D) * (1 - 0.1 * EnchantmentHelper.getEnchantmentLevel(ModEnchantments.ULTIMATE_WISE.get(), stack));
	}

	public double getAbilityCost(ItemStack stack) {
		return 300.0D * (1 - 0.1 * EnchantmentHelper.getEnchantmentLevel(ModEnchantments.ULTIMATE_WISE.get(), stack));
	}
